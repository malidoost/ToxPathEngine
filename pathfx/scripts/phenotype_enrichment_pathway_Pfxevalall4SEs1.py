...
dtd = pickle.load(open('../rscs/Pfxevalall4SEs1_dint.pkl','rb'))
db = pickle.load(open('../rscs/Pfxevalall4SEs1_dbid2name.pkl','rb')) # Drugbank ID to name
...
def do_network(tlist,aname,outdir,dname,doCluster):
	...
	cmd = 'python ../scripts/get_network_associations_Pfxevalall4SEs1.py -f %s -a %s -d %s -n %s' % (froot,dname,outdir,len(tlist))
	os.system(cmd)
